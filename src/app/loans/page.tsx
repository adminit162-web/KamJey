  ...
  return <main className="route-page">
    ...
    <div className="page-toolbar">
      <div className="filter-tabs">{["All", "Interest-free", "Active", "Overdue", "Paid"].map((item) =>
        <button key={item} onClick={() => setFilter(item)} className={filter === item ? "active" : ""}>{t(item === "Paid" ? "Paid off" : item)}<span>{loans.filter((loan) => matchesFilter(loan, item)).length}</span></button>
      )}</div>
