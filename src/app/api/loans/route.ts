...
export async function GET() {
  try {
    ...
    const rows = await sql`select l.id, b.full_name as borrower, l.principal, l.current_principal, l.accrued_interest, l.interest_due_since, l.monthly_interest_rate as rate, l.start_date, l.next_payment_date, l.payment_day, l.status, coalesce(sum(p.amount), 0) as paid, coalesce(sum(p.interest_amount), 0) as interest_paid, coalesce(sum(p.principal_amount), 0) as principal_paid from loans l join borrowers b on b.id = l.borrower_id left join payments p on p.loan_id = l.id group by l.id, b.full_name order by l.next_payment_date asc`;
