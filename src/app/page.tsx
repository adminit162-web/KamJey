...
export default function Home() {
  ...
  const [adjustSplit, setAdjustSplit] = useState(false);
  ...
  const upcomingInterest = paymentLoan && paymentDate < paymentLoan.nextPayment.slice(0, 10) ? Math.max(0, Math.round((paymentLoan.currentPrincipal * paymentLoan.rate / 100 + paymentLoan.nextInterestAdjustment) * 100) / 100) : 0;
  const interestAllocation = adjustSplit ? Number(customInterest) || 0 : Math.min(enteredPayment, interestOnPaymentDate);
  const splitInvalid = adjustSplit && (customInterest === "" || interestAllocation < 0 || interestAllocation > enteredPayment || interestAllocation > interestOnPaymentDate + upcomingInterest || enteredPayment - interestAllocation > (paymentLoan?.currentPrincipal || 0));
  ...
  function openPayment(loan: Loan) { setPaymentLoan(loan); setLoadError(""); setAdjustSplit(false); setCustomInterest(""); setPaymentAmount(""); setPaymentDate(localToday()); }
  ...
  async function recordPayment(event: React.FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!paymentLoan || paymentSavingRef.current || splitInvalid) return;
    paymentSavingRef.current = true; setPaymentSaving(true);
    try {
      const data = new FormData(event.currentTarget);
      const response = await fetch(`/api/loans/${paymentLoan.id}/payments`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ amount: Number(data.get("amount")), paidAt: data.get("paidAt"), method: data.get("method"), ...(adjustSplit ? { interestAmount: Number(customInterest), interestDueDate: paymentLoan.nextPayment.slice(0, 10) } : {}) }) });
      const body = await response.json();
  ...
  return <main className="dashboard-page"><header><div><p className="eyebrow">{t("Loan management")}</p><h1>{t("Welcome to KamJey.")}</h1></div><button onClick={() => setShowForm(true)} className="primary-button"><span>＋</span> {t("New loan")}</button></header>
    ...
    {paymentLoan && <Modal close={() => { if (!paymentSaving) setPaymentLoan(null); }}><form onSubmit={recordPayment}><ModalTitle eyebrow={t("RECORD PAYMENT")} title={paymentLoan.borrower} close={() => { if (!paymentSaving) setPaymentLoan(null); }}/><div className="payment-summary three-up"><span>{t("Principal remaining")}<strong>{money(paymentLoan.currentPrincipal)}</strong></span><span>{t("Interest due on selected date")}<strong>{money(interestOnPaymentDate)}</strong></span><span>{t("Next full-month interest")}<strong>{money(fullMonthInterest(paymentLoan))}</strong></span></div><label>{t("Amount received")}<input required autoFocus name="amount" type="number" min="0.01" max={paymentLoan.currentPrincipal + interestOnPaymentDate + (adjustSplit ? upcomingInterest : 0)} step="0.01" value={paymentAmount} onChange={(event) => setPaymentAmount(event.target.value)} /></label><div className="form-grid"><label>{t("Payment date")}<input required name="paidAt" type="date" value={paymentDate} onChange={(event) => { setPaymentDate(event.target.value); setAdjustSplit(false); setCustomInterest(""); }}/></label><label>{t("Method")}<select name="method"><option value="Cash">{t("Cash")}</option><option value="Bank transfer">{t("Bank transfer")}</option><option value="Other">{t("Other")}</option></select></label></div><div className="allocation-controls"><button type="button" className="split-toggle" aria-expanded={adjustSplit} onClick={() => { setCustomInterest(String(Math.min(enteredPayment, interestOnPaymentDate))); setAdjustSplit(!adjustSplit); }}>{t(adjustSplit ? "Use automatic split" : "Adjust split")}</button></div>{adjustSplit && <div className="split-editor"><label>{t("To interest")}<input required type="number" min="0" max={Math.min(enteredPayment, interestOnPaymentDate + upcomingInterest)} step="0.01" value={customInterest} onChange={(event) => setCustomInterest(event.target.value)}/></label><p>{t("The rest goes to principal.")}</p>{upcomingInterest > 0 && <p>{t("Upcoming interest due {date}: {amount}", { date: displayDate(paymentLoan.nextPayment), amount: money(upcomingInterest) })}</p>}{interestAllocation > interestOnPaymentDate && <p>{t("Early interest payment: {amount}", { amount: money(interestAllocation - interestOnPaymentDate) })}</p>}{splitInvalid && <p role="alert">{t("Check the split: interest must fit the payment and unpaid interest; principal cannot exceed the remaining balance.")}</p>}</div>}<div className="allocation-preview"><div><span>{t("This payment")}</span><strong>{money(enteredPayment)}</strong></div><div><span>{t("To interest")}</span><strong>{money(interestAllocation)}</strong></div><div><span>{t("To principal")}</span><strong>{money(principalAllocation)}</strong></div><div className="principal-after"><span>{t("Principal after payment")}</span><strong>{money(principalAfterPayment)}</strong></div></div><p className="allocation-note">{t(adjustSplit ? "Your adjusted split will be saved. Early interest counts toward the displayed due date." : "Interest is paid first. Only the remaining amount reduces the principal and lowers future monthly interest.")}</p><p role="alert" className="payment-form-error">{loadError}</p><button className="primary-button submit" disabled={paymentSaving || splitInvalid}>{t(paymentSaving ? "Saving…" : "Record payment")}</button></form></Modal>}
