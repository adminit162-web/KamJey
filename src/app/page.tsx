...
type LoanFilter = "All" | "Attention" | "Active" | "Paid";
...
export default function Home() {
  ...
  const visibleLoans = useMemo(() => {
    ...
    return loans.filter((loan) => {
      ...
      const matchesFilter = loanFilter === "All" || loanFilter === "Attention" && (loan.status === "Due soon" || loan.status === "Overdue") || loan.status === loanFilter;
  ...
  return <main className="dashboard-page"><header><div><p className="eyebrow">Loan management</p><h1>Welcome to KamJey.</h1></div><button onClick={() => setShowForm(true)} className="primary-button"><span>＋</span> New loan</button></header>
      ...
      <div className="portfolio-toolbar"><div className="portfolio-filters" role="group" aria-label="Filter loans">{(["All", "Attention", "Active", "Paid"] as LoanFilter[]).map((filter) => <button type="button" className={loanFilter === filter ? "active" : ""} key={filter} onClick={() => setLoanFilter(filter)}>{filter}</button>)}</div><label className="portfolio-search"><span aria-hidden="true">⌕</span><input value={loanSearch} onChange={(event) => setLoanSearch(event.target.value)} placeholder="Search borrower or loan ID" aria-label="Search loans" /></label></div>
      <div className="portfolio-table"><div className="portfolio-table-head"><span>Borrower</span><span>Outstanding balance</span><span>Monthly interest</span><span>Next payment</span><span>Status</span><span>Manage</span></div>
        {visibleLoans.length ? visibleLoans.map((loan) => {
          ...
          return <article className="portfolio-row" key={loan.id}>
            ...
            <div className="portfolio-interest"><strong>{money(monthlyInterest)}</strong><small>{loan.rate}% monthly</small></div>
            <div className="portfolio-due"><strong>{readableDate(loan.nextPayment)}</strong><small>{statusText}</small></div>
