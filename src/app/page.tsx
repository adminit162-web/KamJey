...
export default function Home() {
  ...
  return <main className="dashboard-page"><header><div><p className="eyebrow">{t("Loan management")}</p><h1>{t("Welcome to KamJey.")}</h1></div><button onClick={() => setShowForm(true)} className="primary-button"><span>＋</span> {t("New loan")}</button></header>
      <div className="analytics-grid">
        ...
        <DonutCard title={t("Loan status")} copy={t("Current portfolio health")} centerLabel={t("Total loans")} centerValue={String(loans.length)} segments={[{ label: t("Overdue"), value: totals.statuses.overdue, color: "#d95f4f" }, { label: t("Due soon"), value: totals.statuses.dueSoon, color: "#e4a03c" }, { label: t("Active"), value: totals.statuses.active, color: "#3f8b68" }, { label: t("Paid"), value: totals.statuses.paid, color: "#7890a3" }]}/>
      ...
      <div className="portfolio-toolbar"><div className="portfolio-filters" role="group" aria-label={t("Loans")}>{(["All", "Attention", "Interest-free", "Active", "Paid"] as LoanFilter[]).map((filter) => <button type="button" className={loanFilter === filter ? "active" : ""} key={filter} onClick={() => setLoanFilter(filter)}>{t(filter)}</button>)}</div><label className="portfolio-search"><span aria-hidden="true">⌕</span><input value={loanSearch} onChange={(event) => setLoanSearch(event.target.value)} placeholder={t("Search borrower or loan ID")} aria-label={t("Search borrower or loan ID")} /></label></div>
      <div className="portfolio-table"><div className="portfolio-table-head">{["Borrower", "Outstanding balance", "Interest due", "Due date", "Status", "Manage"].map((label) => <span key={label}>{t(label)}</span>)}</div>
        {visibleLoans.length ? visibleLoans.map((loan) => {
          ...
          const statusText = loan.status === "Paid" ? t("Paid in full") : days < 0 ? t("{days} day(s) overdue", { days: Math.abs(days) }) : days === 0 ? t("Due today") : days <= 7 ? t("Due in {days} day(s)", { days }) : t("On track");
          ...
          return <article className="portfolio-row" key={loan.id}>
            ...
            <div className="portfolio-balance"><strong>{money(loan.currentPrincipal + loan.accruedInterest)}</strong><small>{t("{principal} principal · {interest} interest", { principal: money(loan.currentPrincipal), interest: money(loan.accruedInterest) })}</small><div className="portfolio-progress" aria-label={`${Math.round(repaymentProgress)}%`}><i style={{ width: `${repaymentProgress}%` }} /></div></div>
            <div className={`portfolio-interest ${loan.rate === 0 ? "interest-free" : ""}`}><strong>{money(displayedInterestDue(loan))}</strong><small>{loan.rate === 0 ? t("Interest-free loan") : loan.accruedInterest > 0 ? t("Accrued and unpaid") : t("{rate}% · then {amount}/month", { rate: loan.rate, amount: money(fullMonthInterest(loan)) })}</small></div>
            <div className="portfolio-due"><strong>{readableDate(dueReference)}</strong><small>{statusText}</small></div>
            <div className="portfolio-status"><span className={`status-pill ${loan.status.toLowerCase().replace(" ", "-")}`}>{t(loan.status)}</span></div>
